...
func TestBigChase(t *testing.T) {
	...
	start := time.Now()
	p := ParsePattern("gbuggugwwurbbbggrrwbgubrgbgwwwbwurbuurgrburbbrgbrg")
	rack.NumberOfSolutions(p)
	duretion := time.Since(start)
	if duretion > time.Duration(1000000000) {
		t.Errorf("call took to long")
	}
}
